...
Credits: cardano-apexpool

The transaction requires use of the witness-override and witness signing feature of cardano-cli

High level steps
create 3 addresses - payment1, payment2, payment3 verification key, signing key, staking key
2. create a 4th user who will get paid - payment4 verification key, signing key, staking key
generate a key hash for the verification keys for payment1,payment2, payment3.
create a multisig script which stipulates that all 3 addresses need to sign the transaction
create a script address for this script.
fund it with testAda
check if funds received in the script address by querying for it's utxos
create variable to store txid and index of the utxo
build a transaction with witness-override set to 3
have each of the users payment1,2,3 witness this transaction
assemble the transaction to use the raw transaction, witness files together as a signed transaction
submit the transaction.
verify the balance of address payment4
Actual Steps
...
NOTE: If you don't replace with an actual key hash, then your script might fail with a cryptic error!
{
  "type": "all",
  "scripts":
  [
